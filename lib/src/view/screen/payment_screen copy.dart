...
class _BuyNowScreenState extends State<BuyNowScreen> {
  ...
  void _processPayment() {
    if (_selectedMethod == PaymentMethod.momo) {
      ...
      // Handle other payment methods (e.g., PayPal) here
      // Add your payment processing logic for other methods
      // For example, navigate to a new screen after successful payment
  ...
  Future<void> changeprice() async {
    ...
    num? temprice = num.parse(pref.getString('buynow')!);
    print('temp : $temprice');
    num? nullableNum = temprice / 23000;
    int? roundedInt = nullableNum.round().toInt();
    if (roundedInt < 1) {
      roundedInt = 1;
    }
    setState(() {
      paypalprice = roundedInt;
    });
  }

  Future<num> test() async {
    SharedPreferences pref = await SharedPreferences.getInstance();
    num temp1 = num.parse(pref.getString('buynow')!);
    return temp1;
  }
          ...
          child: Column(
            ...
            children: [
              ...
              Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  // _buildOrderItem('Sách', 'x 1', '50,000 vnd', context),
                  // _buildOrderItem('Sách', 'x 1', '50,000 vnd', context),
                  const SizedBox(height: 10),
